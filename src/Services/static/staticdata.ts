...
export class Staticdata {
  ...
  getAllProduct():Iproducts[]{
    return this.productList;
  }

  getProductByCat(name:string):Iproducts[]{
    return (this.filterList = this.productList.filter((prd)=>prd.category === name));
  }

  getProductById(id:number):Iproducts | null{
    let product =  this.productList.find((p)=>p.id === Number(id));
  ...
  editProduct(id:number , pro:Iproducts)
  {
  ...
  deleteById(id:number){
    let product =  this.productList.find((p)=>p.id === Number(id));
    let index = this.productList.indexOf(product as Iproducts);
    if (index !== -1) {
